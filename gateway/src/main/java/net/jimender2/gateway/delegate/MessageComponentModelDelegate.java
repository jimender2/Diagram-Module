package net.jimender2.gateway.delegate;
...
public class MessageComponentModelDelegate extends ComponentModelDelegate {
    ...
    protected void onStartup() {
        // Called when the Gateway's ComponentModel starts. The start itself happens
        // when the client project is
        // loading and includes an instance of the the component type in the page/view
        // being started.
    ...
    protected void onShutdown() {
        // Called when the component is removed from the page/view and the model is
        // shutting down.
        log.infof("Shutting down delegate for '%s'!", component.getComponentAddressPath());
    }

    /**
     * Use the {@link ScriptCallable} annotation to mark a public method accessible
     * to script authors interacting
     * with your component on the backend.
     *
     * Use of {@link PyArgumentMap} and the {@code (PyObject[], String[])} signature
     * is optional, but encouraged, as
    ...
    public void print(PyObject[] pyArgs, String[] keywords) {
        PyArgumentMap argumentMap = PyArgumentMap.interpretPyArgs(pyArgs, keywords, MessageComponentModelDelegate.class,
                "print");
    ...
    // when a ComponentStoreDelegate event is fired from the client side, it comes
    // through this method.
    @Override
    ...
    // not necessary to override for our use case, just here for informational
    // purposes
    @Override
