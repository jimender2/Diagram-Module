package net.jimender2.designer.component;
...
/**
 * Design delegates provide an opportunity to provide a user interface for
 * configuring a component. This interface
 * is generally a supplement to editing properties in the property tree editor
 * provided to all Perspective Components
 * in the designer.
 *
 *
 */
public class TagCountDesignDelegate implements ComponentDesignDelegate {
    ...
    /**
     * Custom configuration panel that is activated/displayed when there is a single
     * component selection. These
     * panels are automatically subscribed to the active
     * {@link ViewResourceEditor}'s {@link EventBus},
     * which fires a {@link SelectionPropertyUpdateEvent} when changes to the
     * selected component's props occur. This
     * provides a convenient mechanism for keeping UI up to date with changes to the
     * props that might originate from
     * direct edits in the PropertyTree editor UI, bindings, etc.
     *
     * To subscribe to these events simply implement a method that takes a single
     * {@link SelectionPropertyUpdateEvent}
     * parameter, and is annotated with {@link Subscribe} as demonstrated below.
     *
     * These delegates exist only for the duration of the current selection, and are
     * automatically unsubscribed from the
     * EventBus when the UI is removed upon a change in selection.
     *
     * In this simple example, we add a supplementary field for configuring the
     * 'interval' property of our TagCount
     * component.
     */
    private static class TagCountComponentConfigPanel extends JPanel {
        ...
        /**
         * Does some basic checks and returns a JsonObject containing a pure property
         * tree, without the qualified value
         * encoding.
         * 
         * @return unencoded props in an Optional (if valid and present), otherwise
         *         empty Optional
         */
         ...
         * 
         * @return interval value currently in props, or null if missing or invalid
         *         value.
         */
        @Nullable
        ...
        /**
         * Subscribe to change events for the current selection. This event fires when
         * there are changes/updates to the
         * property values of the current selection. We don't know where changes
         * originated from, just that they
         * occurred.
         */
        @Subscribe
        public void onComponentPropChange(SelectionPropertyUpdateEvent event) {
            // property changes come as qualified values. We use this util to get a 'pure'
            // JsonElement that does not
            ...
            if (changes != null && changes.isJsonObject()) {
                ...
                // anchor is the first selected element. But this config UI only loads for
